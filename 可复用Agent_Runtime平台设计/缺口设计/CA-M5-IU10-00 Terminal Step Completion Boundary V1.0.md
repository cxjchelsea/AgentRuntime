...
# 17. Independent Review decision

Independent Review 对以下边界做了累计复核：

~~~text
Scheduler decision -> typed pending-skip authority
PENDING -> SKIPPED lifecycle persistence
IU6 finalization -> running Step completion
PARTIAL_SUCCESS -> SUCCESS lifecycle + degraded evidence
degraded dependency fail-closed
terminal reason / degraded recovery compatibility
control-terminal payload -> recovery snapshot compatibility
finalization / observation exact consistency
current_step pointer consistency
~~~
...
下一步必须执行四项门禁；在 Verification Closure 前不得写：
